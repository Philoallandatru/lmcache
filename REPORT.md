...
| 维度 | 是否受影响 | 说明 |
|---|---|---|
| Phase7 multiprompt replay_p0 spread 2.1s | ✅ **仍有效** | BIWIN NVMe sequential read 本身就快 (4.65 GB/s fio),4 盘 L3 真读盘的硬件差异理论上 < 1%。**盘差结论 (BIWIN 最快 / WDC 最慢) 在真 L3 reload 场景下成立**,因为读取是 L3 reader 异步从 4 盘各自 mount point 读 |
...
> ⚠️ **v3 (2026-06-15) 验证**:ranking 不变,但 spread 从 v2 的 2098ms (2.22×) 缩小到 980ms (1.59×)。详见 [docs/hicache-phase7-v3-validation-2026-06-15.md](./docs/hicache-phase7-v3-validation-2026-06-15.md)。核心叙事保持:**BIWIN 最快 / WDC 最慢 / 盘差 1.5-2× 量级**。
...
**核心数据**: sglang 0.5.13 + Qwen3-4B + 20 prompts (140K tokens > L2 41K 容量) + replay p0, 4 盘 spread 0.98 秒 (v3 复现)。v2 (06-14) spread 2.1s 偏大,主因 WDC v2 跑 3.82s,v3 跑 2.64s。
...
| 盘 | Phase2 写峰值 (mount 修正前, BIWIN 重复) | Phase6 fio 顺序读 | **Phase7 v3 L3 reload** | Phase4 14B 写 |
|---|---:|---:|---:|---:|
| BIWIN ext4 | 5284 MB/s (实际 BIWIN 自身) | 4.77 GB/s | **1.66s** 🥇 (page cache hit) | 790 MB/s |
| WDC NTFS | 8004 (实际 BIWIN 重复) | 2.63 GB/s | 2.64s ⚠️ (真盘读) | 1100 (实际 BIWIN) |
| Seagate NTFS | 8106 (实际 BIWIN 重复) | 3.03 GB/s | 2.43s | 1100 (实际 BIWIN) |
| ZHITAI NTFS | 4498 (实际 BIWIN 重复) | 3.62 GB/s | 2.55s | 1101 (实际 BIWIN) |
| **spread** | — | **1.81×** | **0.98s (1.59×)** | — |

### 4.2 选型矩阵

| 场景 | 推荐 | 理由 |
|---|---|---|
| **单盘 + 大模型 + 频繁 reload** | 🥇 **BIWIN** | v3 1.66s 最快 (page cache + 系统盘优势) |
| **多盘 + 长 prefix + 慢 reload 可接受** | 🥈 **Seagate / ZHITAI** | v3 2.43-2.55s, 慢但 931G 容量, 适合偶发 reload |
| **高并发 L3 write (系统 bootstrap 阶段)** | 🥉 **Seagate** | 8106 MB/s 写峰值最高 (Phase2 数据, mount 修正后待验证) |
| **单盘 + 预算敏感** | ⚠️ **WDC** | v3 2.64s 仍然最慢, 1.6× overhead, 大 L3 部署不推荐 |
| **多卡 TP=2 + 14B-AWQ** | 🥇 **BIWIN + 任意 NTFS** | 14B-AWQ write_through 4 盘 1.10 GB/s 写峰值几乎一致 (Phase4), 容量优先 BIWIN |
...
- ✅ **Phase7 v3** (4B 20 prompts + replay_p0, 真 4 盘) — spread **980ms (1.59×)** vs v2 2098ms (2.22×)。**Ranking 不变** (BIWIN < Seagate ≈ ZHITAI < WDC),WDC 仍是 v3 最慢 (2.64s)。详见 [hicache-phase7-v3-validation-2026-06-15.md](./docs/hicache-phase7-v3-validation-2026-06-15.md)
...
| 文件 | 故事 |
|---|---|
| [docs/io-profiling-plots-2026-06-15.md](./docs/io-profiling-plots-2026-06-15.md) | **图索引 + 故事线详解** (生成命令见 README) |
...
| `results/plots/06_cache_hit_vs_device.png` | **核心图**: cache hit vs device latency — 真读盘 2.22× spread, cache hit < 1% |
...
| `results/plots/10_multiprompt_modes.png` | multiprompt 3 模式对比 — WDC replay 3.82s vs BIWIN 1.72s |
| `results/plots/13_p5_l3_replay.png` | P5 3 policy × 4 盘对比 — WB TTFT优势 / WTS 更慢 / WB OOM |

> ⚠️ `results/plots/*.png` **不入 git** (`.gitignore` 已配置),跑 `python scripts/plot_io_data.py` 一键生成。需要先 `source ~/llm/.venv/bin/activate` (matplotlib 依赖)。|
