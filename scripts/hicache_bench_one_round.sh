...
# Phase7 多 prompt 累积测: NUM_PROMPTS=20 + REPLAY_PROMPT_ID=0 让 L2 装满后 L3 reload
NUM_PROMPTS=${NUM_PROMPTS:-0}
REPLAY_PROMPT_ID=${REPLAY_PROMPT_ID:--1}
...
echo "--- hicache_load_test.py start (clients=$CONCURRENT_CLIENTS drop_every=$DROP_EVERY_ROUND prompt=$PROMPT_TOKENS num_prompts=$NUM_PROMPTS replay_id=$REPLAY_PROMPT_ID) ---"
LOAD_TEST_FLAGS="--prompt-tokens $PROMPT_TOKENS --output-tokens 64 --request-rate 1.0"
# num_prompts > 0 走 multi-prompt 模式 (覆盖 num-rounds)
if [ "$NUM_PROMPTS" -gt 0 ]; then
    LOAD_TEST_FLAGS="$LOAD_TEST_FLAGS --num-prompts $NUM_PROMPTS"
    if [ "$REPLAY_PROMPT_ID" -ge 0 ]; then
        LOAD_TEST_FLAGS="$LOAD_TEST_FLAGS --replay-prompt-id $REPLAY_PROMPT_ID"
    fi
else
    # 旧 num-rounds 同 prompt 模式
    LOAD_TEST_FLAGS="$LOAD_TEST_FLAGS --num-rounds 6"
